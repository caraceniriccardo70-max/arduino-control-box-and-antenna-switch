...
final String APP_VERSION = "2.2 - Raspberry Pi";
...
// ⭐ SCALA UI - Regolabile con frecce UP/DOWN
float uiScale = 0.6;  // Default per Raspberry Pi 7" (800x480)
...
void setup() {
  size(800, 480);  // ⭐ Raspberry Pi 7" touchscreen
  smooth(8);
  frameRate(60);
  surface.setTitle(APP_NAME + " v" + APP_VERSION);
  surface.setResizable(true);  // ⭐ Aggiunto
  ...
  // ⭐ Aggiunte info scala UI
  addDebugLog("Risoluzione: " + width + "x" + height);
  addDebugLog("Scala UI: " + nf(uiScale, 1, 2));
  addDebugLog("Usa frecce ↑↓ per regolare la scala");
...
// ⭐ Helper per controllo mouse con scala
...
void draw() {
  ...
  popMatrix();  // ⭐ Chiudi scala
  
  // ⭐ Indicatore scala (non scalato)
  drawScaleIndicator();
...
void drawScaleIndicator() {
  // Sfondo
  fill(theme.panel, 200);
  noStroke();
  rect(width - 120, height - 40, 110, 30, 5);
  ...
  textSize(11);
  textAlign(LEFT, CENTER);
  text("Scala: " + nf(uiScale, 1, 2) + "x", width - 110, height - 25);
  
  // Icone frecce
  fill(theme.textDim);
  textSize(9);
  text("↑↓", width - 30, height - 25);
}
...
void keyPressed() {
  // ⭐ Regola scala con frecce UP/DOWN
  if (keyCode == UP) {
    uiScale = min(uiScale + SCALE_STEP, MAX_SCALE);
    addDebugLog("Scala UI: " + nf(uiScale, 1, 2));
    return;
  } else if (keyCode == DOWN) {
    uiScale = max(uiScale - SCALE_STEP, MIN_SCALE);
    addDebugLog("Scala UI: " + nf(uiScale, 1, 2));
    return;
  }
